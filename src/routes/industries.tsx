...
const SERVICE_APPLICATIONS = [
  { id: "hvac", title: "HVAC", icon: Wind, content: "Centralized Chilled Water Plants, VRF Systems, Ducting, and Precision Cooling for Data Centers." },
  { id: "cryo", title: "Cryogenic", icon: Snowflake, content: "VIV Pipelines, Liquid Oxygen Tanks, Vaporizers, and Vacuum Jacketed Valve Boxes." },
  { id: "fire", title: "Fire Fighting", icon: Flame, content: "Hydrant Systems, Automated Sprinklers, CO2 Flooding, and Fire Alarm Integration." },
  { id: "elec", title: "Electrical", icon: Zap, content: "HT/LT Panels, Transformer Installation, Industrial Wiring, and Lighting Protection." },
];
...
const FAQS = [
  { q: "Which industries do you specialize in?", a: "We specialize in Industrial Manufacturing, Healthcare, Commercial Real Estate, and Food Processing, though our systems are scalable for any enterprise sector." },
  { q: "Do you handle large industrial projects?", a: "Yes, SP Engineering is equipped to handle turnkey industrial projects, from greenfield gas plants to mega manufacturing units." },
  { q: "Are AMC services available?", a: "We provide comprehensive Annual Maintenance Contracts to ensure your systems remain compliant and efficient 24/7." },
  { q: "Do you provide customized engineering solutions?", a: "Every project is unique. Our design team creates bespoke blueprints tailored specifically to your site's thermal and load requirements." },
  { q: "Are your services safety compliant?", a: "We adhere strictly to NBC, ISO, and industry-specific safety standards (like ASHRAE for HVAC or NFPA for Fire Fighting)." },
];
...
const CardGradientBorder = ({ children }: { children: React.ReactNode }) => (
  <div className="relative group p-[1px] rounded-sm overflow-hidden bg-[#EBCDCB]/10 hover:bg-gradient-to-br hover:from-[#981618] hover:to-[#A27273] transition-all duration-500">
    <div className="bg-[#160E0E] rounded-sm h-full w-full">
      {children}
    </div>
  </div>
);
    ...
    >
      ...
      <div className="relative z-10 mt-12 grid lg:grid-cols-2 gap-12 items-center mb-32">
        ...
        >
          <p className="text-lg sm:text-xl font-light leading-relaxed mb-8">
            <span className="text-white font-semibold">SP Engineering</span> provides specialized HVAC, Cryogenic, Fire Fighting, and Electrical solutions for diverse industries with strong focus on safety, performance, and quality execution.
          </p>
          <div className="grid grid-cols-2 gap-6">
            {STATS.map((stat, i) => (
              <div key={i} className="border-l-2 border-[#981618] pl-4">
                <div className="text-2xl sm:text-3xl font-bold text-white mb-1">{stat.value}</div>
                <div className="text-[10px] uppercase tracking-widest text-[#A27273] font-bold">{stat.label}</div>
              </div>
            ))}
          </div>
        </motion.div>
        ...
        <div className="flex justify-center lg:justify-end gap-4 relative">
          ...
          <div className="grid grid-cols-2 gap-4">
            <a href="#explore" className="px-8 py-4 bg-[#981618] text-white font-bold text-xs uppercase tracking-widest shadow-[0_0_20px_#98161833] hover:shadow-[0_0_40px_#98161866] transition-all">Explore Services</a>
            <a href="/contact" className="px-8 py-4 bg-transparent border border-[#EBCDCB]/20 text-white font-bold text-xs uppercase tracking-widest hover:bg-[#EBCDCB]/5 transition-all">Contact Us</a>
          </div>
        </div>
      </div>
      ...
      <div id="explore" className="relative z-10 mb-32">
        <div className="text-center mb-16">
          <h2 className="text-xs uppercase tracking-[0.3em] text-[#981618] font-black mb-4">Enterprise Sectors</h2>
          <p className="text-2xl sm:text-4xl font-bold text-white tracking-tight">Industrial Strength Coverage</p>
        </div>
              ...
              >
                <CardGradientBorder>
                  <div className="p-8 h-full flex flex-col group/item relative overflow-hidden">
                    ...
                    <h3 className="text-lg font-bold text-white mb-3 tracking-wide">{industry.title}</h3>
                    <p className="text-xs text-[#A27273] leading-relaxed mb-6 font-light">{industry.desc}</p>
      ...
      <div className="relative z-10 mb-32 bg-[#160E0E]/60 border border-[#EBCDCB]/10 p-8 sm:p-12 rounded-sm backdrop-blur-md">
        <div className="text-center mb-12">
          <h2 className="text-xs uppercase tracking-[0.3em] text-[#981618] font-black mb-4">Core Applications</h2>
          <p className="text-xl sm:text-3xl font-bold text-white">System Deployments by Discipline</p>
        </div>
      ...
      <div className="relative z-10 mb-32">
        <div className="text-center mb-20">
          <h2 className="text-xs uppercase tracking-[0.3em] text-[#981618] font-black mb-4">Engineering Workflow</h2>
          <p className="text-2xl sm:text-4xl font-bold text-white tracking-tight">From Briefing to Maintenance</p>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-8">
            {TIMELINE.map((item, i) => (
              ...
              >
                ...
                <h4 className="text-sm font-bold text-white uppercase tracking-widest mb-2">{item.stage}</h4>
      ...
      <div className="relative z-10 mb-32 grid md:grid-cols-2 lg:grid-cols-3 gap-6">
        {[
          ...
          <div key={i} className="p-8 bg-[#160E0E]/40 border border-[#EBCDCB]/5 hover:border-[#98161833] transition-all rounded-sm flex items-center gap-6 group">
            <div className="text-[#981618] group-hover:scale-110 transition-transform"><feature.icon size={32} /></div>
      ...
      <div className="relative z-10 mb-32 overflow-hidden select-none opacity-50">
        <div className="flex space-x-8 animate-marquee mb-4">
          {Array(2).fill(["RELIANCE", "TATA", "GODREJ", "L&T", "SIEMENS", "ADANI"]).flat().map((client, i) => (
            <div key={i} className="px-10 py-6 border border-[#EBCDCB]/10 bg-[#160E0E] text-white font-mono text-xs uppercase tracking-[0.5em] flex items-center justify-center grayscale hover:grayscale-0 transition-all duration-500">
              {client}
            </div>
          ))}
        </div>
        <div className="flex space-x-8 animate-marquee--reverse">
          {Array(2).fill(["HOSPITAL CORE", "CHAKAN IND", "APEX FOODS", "TITAN GAS", "METRO RAIL", "NHPC"]).flat().map((client, i) => (
            <div key={i} className="px-10 py-6 border border-[#EBCDCB]/10 bg-[#160E0E] text-white font-mono text-xs uppercase tracking-[0.5em] flex items-center justify-center grayscale hover:grayscale-0 transition-all duration-500">
              {client}
            </div>
          ))}
      ...
      <div className="relative z-10 max-w-3xl mx-auto mb-32">
        <div className="text-center mb-16">
          <h2 className="text-xs uppercase tracking-[0.3em] text-[#981618] font-black mb-4">Q&A Matrix</h2>
          <p className="text-xl sm:text-3xl font-bold text-white tracking-tight">Industry-Specific Resolutions</p>
        </div>
        ...
        <div className="space-y-4">
          {FAQS.map((faq, index) => (
            <div key={index} className="border border-[#EBCDCB]/10 rounded-sm bg-[#160E0E]/30 overflow-hidden">
              ...
              >
                <span className="text-xs sm:text-sm font-semibold text-white uppercase tracking-wider">{faq.q}</span>
      ...
      <div className="relative z-10 bg-gradient-to-b from-[#160E0E]/80 to-[#221011]/80 border border-[#981618]/40 p-12 sm:p-20 rounded-sm text-center shadow-[0_0_50px_#9816181a] mb-20">
        <h2 className="text-2xl sm:text-5xl font-bold text-white tracking-tight mb-6">Looking For Engineering Solutions For Your Industry?</h2>
        <div className="flex flex-wrap justify-center gap-6">
          ...
          <a href="/contact" className="px-10 py-4 bg-transparent border border-[#EBCDCB]/20 text-white font-bold text-xs uppercase tracking-widest hover:bg-white/5 transition-all">
