...
const PROJECTS_DATA = [
  ...
  },
  {
    ...
    status: "Completed",
    image: "https://images.unsplash.com/photo-1599740831464-5cbe1d14f87a?auto=format&fit=crop&w=800&q=80",
...
const TESTIMONIALS = [
  {
    quote: "SP Engineering delivered our cryogenic infrastructure arrays ahead of schedule. Their parameter verification methods are flawless.",
    author: "Director of Operations",
    company: "Apex Chemical Logistics"
  },
  {
    quote: "The complex HVAC retrofitting handled on our production floor has significantly lowered ambient particle pollution counts.",
    ...
    >
      ...
      <div id="project-grid" className="mb-32 relative z-10">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center border-b border-[#EBCDCB]/10 pb-6 mb-12 gap-6">
          <div>
            <span className="text-[10px] uppercase tracking-widest text-[#981618] font-bold block mb-1">Portfolio Matrix</span>
            <h3 className="text-xl sm:text-3xl font-bold text-white tracking-tight">Verified Engineering Records</h3>
          </div>
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1.5 mb-6">
                      {project.tech.map((t, i) => (
                        <span key={i} className="text-[10px] bg-[#160E0E] text-[#EBCDCB]/80 border border-[#EBCDCB]/5 px-2.5 py-1 rounded-sm font-mono">
                          {t}
                        </span>
                      ))}
                    </div>
      ...
      <div className="mb-32 relative z-10">
        <div className="text-center mb-16">
          <span className="text-[10px] uppercase tracking-widest text-[#981618] font-bold block mb-2">Metrics Analysis</span>
          <h3 className="text-2xl sm:text-4xl font-bold text-white tracking-tight">Deployment Metamorphosis</h3>
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {PROJECTS_DATA.slice(0, 2).map((project) => (
            <div key={project.id} className="bg-[#160E0E]/40 border border-[#EBCDCB]/10 p-6 sm:p-8 rounded-sm backdrop-blur-md">
              <span className="text-xs uppercase text-[#A27273] tracking-widest font-mono block mb-3">{project.title}</span>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="bg-[#160E0E] border border-red-950/40 p-4 rounded-sm">
                  <div className="text-[10px] uppercase tracking-wider text-[#A27273] font-bold mb-1 flex items-center gap-1.5">
                    <div className="w-1.5 h-1.5 rounded-full bg-orange-600" /> Phase 01: Pre-Execution
                  </div>
                  <p className="text-xs text-[#A27273]/90 leading-relaxed font-light">{project.before}</p>
                </div>
                <div className="bg-[#981618]/10 border border-[#981618]/30 p-4 rounded-sm">
                  <div className="text-[10px] uppercase tracking-wider text-white font-bold mb-1 flex items-center gap-1.5">
                    <div className="w-1.5 h-1.5 rounded-full bg-green-500 shadow-[0_0_6px_#22c55e]" /> Phase 04: Commissioned
                  </div>
                  <p className="text-xs text-[#EBCDCB] leading-relaxed font-light">{project.after}</p>
                </div>
              </div>
            </div>
          ))}
      ...
      <div className="mb-32 relative z-10">
        <div className="text-center mb-20">
          <span className="text-[10px] uppercase tracking-widest text-[#981618] font-bold block mb-2">Execution Protocol</span>
          <h3 className="text-2xl sm:text-4xl font-bold text-white tracking-tight">System Engineering Pipeline</h3>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6 relative">
          {[
            { step: "01", name: "Consultation", icon: Briefcase, d: "Parametric mapping alignment." },
            { step: "02", name: "Planning", icon: SlidersHorizontal, d: "Structural calculation loops." },
            ...
            { step: "05", name: "Delivery", icon: CheckCircle2, d: "Formal handover commissioning." }
          ].map((proc, idx) => {
            const IconComp = proc.icon;
            return (
              <div key={idx} className="bg-[#160E0E]/70 border border-[#EBCDCB]/5 p-6 rounded-sm relative group hover:border-[#981618]/30 transition-all">
                <div className="flex justify-between items-center mb-4">
                  <span className="font-mono text-xl font-bold text-[#981618]">{proc.step}</span>
                  <IconComp className="w-4 h-4 text-[#A27273]" />
                </div>
                <h4 className="text-sm font-bold text-white tracking-wide uppercase mb-1">{proc.name}</h4>
      ...
      <div className="mb-32 relative z-10">
        <div className="text-center mb-16">
          <span className="text-[10px] uppercase tracking-widest text-[#981618] font-bold block mb-2">Operational Reach</span>
          <h3 className="text-2xl sm:text-4xl font-bold text-white tracking-tight">Target Sectors Authorized</h3>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
          {INDUSTRIES.map((ind, i) => {
            const Icon = ind.icon;
            return (
              <div key={i} className="bg-[#160E0E]/40 border border-[#EBCDCB]/5 p-6 rounded-sm text-center group hover:border-[#981618]/30 transition-all duration-300 backdrop-blur-sm">
                ...
                <h4 className="text-xs font-bold text-white uppercase tracking-wider">{ind.name}</h4>
              </div>
            );
          })}
        </div>
      ...
      <div className="mb-32 overflow-hidden pointer-events-none relative w-full select-none opacity-40">
        ...
        <div className="flex space-x-8 animate-marquee whitespace-nowrap mb-4">
          {Array(10).fill(["SP INFRA", "APEX LOGISTICS", "TITAN CORP", "NEXUS INDUSTRIAL"]).flat().map((logo, i) => (
            <span key={i} className="font-mono text-xs uppercase tracking-[0.4em] text-[#A27273] border border-[#EBCDCB]/10 px-6 py-2.5 bg-[#160E0E]/50">
              {logo}
            </span>
          ))}
        </div>
       ...
       <div className="flex space-x-8 animate-marquee--reverse whitespace-nowrap">
          {Array(10).fill(["VANGUARD COMPONENT", "CHAKAN SEZ MATRICES", "MUMBAI PETRO", "LOTUS CORE"]).flat().map((logo, i) => (
            <span key={i} className="font-mono text-xs uppercase tracking-[0.4em] text-[#A27273] border border-[#EBCDCB]/10 px-6 py-2.5 bg-[#160E0E]/50">
              {logo}
            </span>
          ))}
      ...
      <div className="relative bg-gradient-to-b from-[#160E0E]/80 to-[#221011]/80 border border-[#981618]/40 p-8 sm:p-16 rounded-sm text-center shadow-[0_0_50px_rgba(152,22,24,0.15)] mb-12 z-10">
        <h2 className="text-2xl sm:text-4xl font-bold text-white tracking-tight mb-4">Let’s Build Your Next Engineering Asset</h2>
        <p className="text-xs sm:text-sm text-[#A27273] max-w-xl mx-auto mb-8 font-light leading-relaxed">
          Connect with our estimation engineers to isolate optimization curves across HVAC, Cryo, or Safety disciplines.
        </p>
        <div className="flex flex-wrap justify-center gap-4">
          <a href="/contact" className="px-8 py-3.5 bg-[#981618] text-white font-semibold text-xs tracking-widest uppercase rounded-sm">
          ...
          >
            ...
            >
              <div className="flex justify-between items-start">
                <div>
                  <span className="text-[10px] font-mono uppercase bg-[#981618] text-white px-2 py-0.5 rounded-sm">{selectedProject.category}</span>
              ...
              <div className="space-y-4 text-xs sm:text-sm">
                <p className="text-[#A27273] leading-relaxed font-light">{selectedProject.desc}</p>
                <div className="p-4 bg-[#160E0E] border border-[#EBCDCB]/5 space-y-2">
                  <span className="text-[10px] uppercase font-bold text-white tracking-widest block">Structural Metadata</span>
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    <div><span className="text-[#A27273]">Location:</span> <span className="text-[#EBCDCB]">{selectedProject.location}</span></div>
                    <div><span className="text-[#A27273]">Status:</span> <span className="text-green-500 font-bold">{selectedProject.status}</span></div>
                  </div>
                </div>
              </div>
            </motion.div>
